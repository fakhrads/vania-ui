...
- Tema terang + gelap, toggle 3 posisi (terang/sistem/gelap) di sidebar; skrip boot di `<head>` (`src/lib/theme.tsx`) mencegah kedip. Font: Hanken Grotesk (teks), Geist Mono (`.num`, angka & ID), Instrument Serif (`.display`, judul & angka utama — satu bobot saja, jangan dipasangi `font-semibold`).
